...
ava_url_list = ["http://www.cntour2.com/"] # 记得要改

# 白名单
white_cat_list = ["viewnews"] # 记得要改
# 黑名单
        ...
        for news in news_list:
            try:
                ...
                tmp_news_info = [news.text.replace("\n", ""),
                                 cat_name,
                                 news.title.split("--")[0].strip().replace("——旅新网","")]  # 单篇新闻的内容、类别、标题，稍后会被追加到news_info中
                news_info.append(list(tmp_news_info))
                print(tmp_news_info)
                tmp_news_info.clear()  # 记得清空临时存储单条新闻的列表
                if len(news_info) >= SAVE_CNT:
                    print("保存", SAVE_CNT, "条新闻")
                    export2excel(news_info, "新闻文本分类算法样本集 - 旅兴网 "+cat_name+".xlsx")  # 保存到对应名称的excel工作簿中
