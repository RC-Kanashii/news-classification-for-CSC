...
def start_job_2():##功能2：支持本地上传csv/xlsx文件，批量输入新闻，并输出新闻分类
    ...
    t2 = st.ScrolledText(window, height=10,font=f1,width=60)
    t2.grid(row=6,column=2)


def start_2():#功能2：支持本地上传csv/xlsx文件，批量输入新闻，并输出新闻分类
    t2.grid(row=6,column=2)#用于输出程序结果，处理后的结果（需要编写）
...
t1=st.ScrolledText(window,height=10,width=80)
t1.grid(row=7, column=0)

l2.grid(row=1,column=2)#功能2的简介
